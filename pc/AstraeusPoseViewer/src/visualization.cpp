...
void drawWorld(HDC dc,const RECT& area,const Pose& pose,bool valid) {
    ...
    if(valid) {
        auto transform=[&](V p) {
            ...
            return V{pose.position[0]+p[0]+q[3]*t[0]+q[1]*t[2]-q[2]*t[1],
                     pose.position[1]+p[1]+q[3]*t[1]+q[2]*t[0]-q[0]*t[2],
                     pose.position[2]+p[2]+q[3]*t[2]+q[0]*t[1]-q[1]*t[0]};
        };
        V corners[8];
        for(int i=0;i<8;++i) corners[i]=transform({(i&1)?0.12f:-0.12f,(i&2)?0.07f:-0.07f,(i&4)?0.06f:-0.06f});
        for(int i=0;i<8;++i) for(int bit=1;bit<=4;bit*=2) if(!(i&bit)) line(corners[i],corners[i|bit],RGB(65,45,140),3);
        line(transform({0,0,0}),transform({0,0,-0.4f}),RGB(230,130,20),3);
    }
    RestoreDC(dc,-1);
}
}
